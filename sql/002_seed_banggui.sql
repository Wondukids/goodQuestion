...
FROM 이야기, (VALUES
  ...
  (3::smallint, 'ch_banggui_daughter_in_law', '방귀쟁이 며느리',
   ...
   '{"PERSPECTIVE": "며느리가 지금 처한 사정을 말한다. 참고 있는 몸 상태든, 말하지 못하는 까닭이든, 둘레 사람이 며느리에게 한 일이든 된다.",
     ...
     "SOLUTION": "며느리가 지금 해 볼 수 있는 행동을 말한다. 방귀를 뀌는 일이든 가족에게 말하는 일이든 된다."}'::jsonb,
  ...
  (5::smallint, 'ch_banggui_father_in_law', '시아버지',
   ...
   '{"PERSPECTIVE": "며느리가 그때 어떤 처지였는지를 말한다. 시아버지 자신의 놀람이나 체면은 시아버지가 이미 스스로 말하는 것이라 이 자리가 아니다.",
     ...
     "REQUEST": "시아버지에게 무엇을 해 달라고, 또는 하지 말아 달라고 말한다."}'::jsonb,
   3::smallint, 5::smallint),

  -- 대화3 · sc_banggui_07 -----------------------------------------------------
  (7::smallint, 'ch_banggui_village_chief', '마을 이장',
   '해마다 배가 열리는데 배나무가 너무 높아 장대로도 나무를 타고도 아무도 따지 못한다.',
   ...
   '{"SOLUTION": "배를 떨어뜨릴 방법을 말한다. 무엇을 써서 떨어뜨릴지가 말에 있어야 한다.",
     ...
     "RESULT": "그 방법을 쓰면 배나무나 마을에 무슨 일이 생길지를 말한다."}'::jsonb,
   3::smallint, 5::smallint),
  ...
  (9::smallint, 'ch_banggui_daughter_in_law', '방귀쟁이 며느리',
   ...
   '{"EMPATHY": "아직 부끄러움이 남은 며느리의 마음을 읽어 말한다.",
     ...
     "SOLUTION": "며느리가 앞으로 무엇을 할지 말한다."}'::jsonb,
   2::smallint, 4::smallint)
