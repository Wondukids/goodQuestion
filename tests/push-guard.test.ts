...
//   2. `tablesFilter` — 그 담을 지나가도 우리 24표 밖을 안 본다 (`우리_표_이름들`)
...
/**
 * `public` 에 남는 엔진 13표 (미션 3표는 sql/005 · 이슈 #17, 리포트 2표는 sql/006 · 이슈 #35,
 * 후활동 단어 표는 sql/007 · 이슈 #42).
 *
...
const 엔진_13표 = [
  ...
  'parent_reports',
  'child_words',
  // 후활동 단어 판정 (이슈 #42). ⚠️ 미션 표 셋과 이름이 비슷해 보여도 다른 활동이다
  // (이야기 **도중**이 아니라 **다 본 뒤**. 말하기 후 활동 명세 3절)
...
describe('표가 어느 스키마에 사는가 (이름만으로는 못 잡는다)', () => {
  ...
  it('엔진 13표는 `public` 에 남는다 — 같이 끌려가지 않았는지', () => {
    for (const 표 of 엔진_13표) {
      expect(표별_스키마.get(표), `${표} 가 사는 스키마`).toBe('public')
    }
  })

  it('⚠️ 24표가 이 둘로 다 갈린다 — 어느 쪽에도 안 든 새 표가 있으면 빨개진다', () => {
    expect([...표별_스키마.keys()].sort()).toEqual([...관리_11표, ...엔진_13표].sort())
  })
})
