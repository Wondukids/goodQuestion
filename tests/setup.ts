...
// 🔴 이 세 그물에는 **구멍이 하나 있었다** — 고친 자리가 여기가 아니라 제품 코드다.
//
// 공급자는 SDK 를 `await import('@google/genai')` 로 가져오는데, 같은 모듈을 **동시에 두 번**
// 동적 import 하면 **두 번째가 vi.mock 을 지나쳐 진짜 모듈을 받는다** (vitest 4.1.10 실측):
//
//     const [가, 나] = await Promise.all([import('@google/genai'), import('@google/genai')])
//     // 가 = 가짜(그물)  ·  나 = 진짜 SDK
//
// 보호자 리포트(이슈 #38)가 두 편을 **동시에** 부르면서 2026-08-15 에 그 구멍으로 진짜
// 제미나이 호출이 나갔다. 여기서 모듈을 미리 불러 굳혀 봤지만 **안 막힌다** — 경합은
// 「등록부에 가짜가 있나」가 아니라 「동시에 두 번 두드리나」에서 난다.
// 그래서 `src/llm/provider/index.ts` 가 `import()` 를 모듈당 한 번만 하도록 고쳤다
// (프라미스를 재사용한다). 그 파일 머리말에 재현과 이유가 있다.
