...
export async function completeRun(
  ...
  //
  // 🔴 `후활동을_기다린다` — 이야기에 말하기 후 활동이 붙어 있으면 여기서 **안 만든다**
  //    (이슈 #43 · 후활동 명세 7.1 F10). 아이가 이야기를 다 본 뒤 활동을 하나 더 하므로,
  //    여기서 만들면 그 결과가 리포트에 못 들어간다. 후활동이 **없는** 이야기는 지금까지처럼
  //    이 자리에서 만들어진다 (F13) — 그런 이야기는 끝 화면에 활동 버튼이 없어 만들라는
  //    신호가 달리 오지 않는다. ⛔ 그래서 이 줄을 지우면 안 된다.
  queueReport(run.session_id, { 후활동을_기다린다: true })
}
