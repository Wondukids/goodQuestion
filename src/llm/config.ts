 ...
 *
 * ⚠️ 팀 repo 착지(이슈 #26 ④)에서 `'lib'` 이 `'src/lib'` 로 바뀌었고, 도메인별 폴더
 *    가르기에서 다시 `'src/llm'` 이 되었다 — 코드가 `web/lib/` → `src/lib/` →
 *    `src/llm/` 로 옮겨졌기 때문이다. 이 한 칸이 어긋나면 `PROJECT_ROOT` 가
 *    **레포 밖**을 가리키고 골든셋 화면이 **조용히 빈다**.
 *    (깊이는 옛 `src/lib/config.ts` 와 같은 두 칸이라 `..`·`..` 횟수는 그대로다.)
 */
const 이_파일_폴더 = import.meta.dirname as string | undefined
const 기준 = 이_파일_폴더 ?? path.join(process.cwd(), 'src', 'llm')

/** 레포 루트. 이 파일은 `<루트>/src/llm/config.ts` 다. */
