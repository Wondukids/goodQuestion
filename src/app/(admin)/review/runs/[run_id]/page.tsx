...
export default async function ReviewPage({
  ...
  return (
    <main className="flex flex-col gap-6">
      ...
      <머리 보기={보기} />
      {보기.turn === null ? (
        <p className="text-xs text-ink-muted">검수할 아이 발화가 없다.</p>
      ) : (
        <>
          <턴이동 보기={보기} />
    ...
    <section className="flex flex-col gap-2">
      <div className="flex flex-wrap items-baseline gap-3">
        <h2 className="font-semibold">
          회차 검수 <span className="font-mono text-xs text-ink-muted">{보기.run.id}</span>
        </h2>
      ...
      {보기.goldenset_stale && (
        <p role="alert" className="border border-warn px-3 py-1 text-xs text-warn">
...
function 판정칸({ 체크리스트 }: { 체크리스트: readonly { code: string; label: string }[] }) {
  return (
    <>
      <fieldset className="border border-divider p-2 text-xs">
        <legend className="text-ink-muted">판정 기준</legend>
        {체크리스트.map((항목) => (
      ...
      <fieldset className="flex flex-wrap gap-4 border border-divider p-2 text-xs">
        <legend className="text-ink-muted">판정</legend>
...
function 분석검수({ run_id, turn }: { run_id: string; turn: ReviewTurnView }) {
  return (
    <section className="flex flex-col gap-2 border border-divider p-3">
      ...
      <p className="font-mono text-xs text-ink-muted">출처: prompts/analysis.md</p>
      <dl className="grid grid-cols-[max-content_1fr] gap-x-3 font-mono text-xs">
        <dt className="text-ink-muted">child_intent</dt>
        <dd>{turn.child_intent ?? 'null'}</dd>
        <dt className="text-ink-muted">main_point</dt>
        <dd className="break-all">{turn.main_point ?? 'null'}</dd>
        <dt className="text-ink-muted">찾은 사고 요소</dt>
        <dd>
          {turn.detected_element_names.length === 0 ? '없음' : turn.detected_element_names.join(', ')}
        </dd>
        <dt className="text-ink-muted">utterance_validity</dt>
      ...
      <form action={analysisScoreAction} className="flex flex-col gap-2">
        ...
        <button type="submit" className="w-fit border border-divider px-3 py-1 font-semibold">
          분석 판정 남기기
        </button>
      </form>
    </section>
...
function 대사검수({ run_id, turn }: { run_id: string; turn: ReviewTurnView }) {
  return (
    <section className="flex flex-col gap-2 border border-divider p-3">
      ...
      <p className="font-mono text-xs text-ink-muted">출처: prompts/character.md</p>
      {/* ⭐ 이 턴에 대사가 없으면 「응답 없음」이다. 다음 턴 대사를 빌려 오지 않는다 (FR-048). */}
      <blockquote className="border-l-2 border-divider pl-2">
...
function 판정이력({ records }: { records: ReviewRecordView[] }) {
  if (records.length === 0) {
    return <p className="text-xs text-ink-muted">아직 사람이 매긴 판정이 없다.</p>
  }
